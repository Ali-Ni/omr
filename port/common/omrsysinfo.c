...
/**
 * Sets the number of active CPUs, a user specified value which overrides all other forms of CPU detection
 * in omrsysinfo_get_number_CPUs_by_type for type OMRPORT_CPU_TARGET.
 * 
 * @param[in] portLibrary The port library.
 * @param[in] number Number of active CPUs.
 */
void
omrsysinfo_set_number_active_CPUs(struct OMRPortLibrary *portLibrary, uintptr_t number)
{
	portLibrary->portGlobals->activeCPUs = number;
}
 ...
 *    overridden by portLibrary->portGlobals->activeCPUs if it is set. 
 *
 * @param[in] portLibrary The port library.
